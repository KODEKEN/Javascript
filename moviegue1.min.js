...
return jQuery.easing.easeOutBounce(x,t*2-d,0,c,d)*0.5+c*0.5+b}});var searchFormConfig={summaryPost:!0,resultThumbnail:!0,fallbackThumb:"https://1.bp.blogspot.com/-41xX9MKBr-w/U1Tt0T8gNRI/AAAAAAAABZw/SocPS0qeq1k/s50-c/Noim.png",thumbSize:50,summaryLength:100};(function(c){var a=c("#bsearch-box"),d=a.find(":text");config=searchFormConfig;a.append('<div id="search-result"></div>');var b=c("#search-result");a.on("submit",function(){var e=d.val();b.slideDown(500,"easeOutBounce").html("Sedang Proses...");c.ajax({url:"/feeds/posts/summary?alt=json-in-script&q="+e+"&max-results=9999",type:"get",dataType:"jsonp",success:function(q){var p=q.feed.entry,m,l,o,n,g="";if(p!==undefined){g="<h4>Hasil Pencarian &quot;"+e+"&quot;</h4>";g+='<a class="close" href="/">&times;</a><ul>';for(var k=0;k<p.length;k++){var f=new RegExp(e,"ig"),o=p[k].title.$t.replace(f,"<mark>"+e+"</mark>");for(var h=0;h<p[k].link.length;h++){if(p[k].link[h].rel=="alternate"){n=p[k].link[h].href}}
if(config.summaryPost===!0){if("content" in p[k]){m=p[k].content.$t}else{if("summary" in p[k]){m=p[k].summary.$t}else{m=""}}}
if(config.resultThumbnail===!0){if("media$thumbnail" in p[k]){l=p[k].media$thumbnail.url.replace(/\/s[0-9]+\-c/g,"/s"+config.thumbSize+"-c")}else{l=config.fallbackThumb}}
m=m.replace(/<\S[^>]*>/g,"");if(m.length>config.summaryLength){m=m.substring(0,config.summaryLength)+"..."}
m=m.replace(f,"<mark>"+e+"</mark>");g+='<li><a href="'+n+'" target="_blank"><span><img src="'+l+'" alt="" /></span><h5>'+o+"</h5></a><p>"+m+"</p></li>"}
g+="</ul>";b.html(g)}else{b.html('<a class="close" href="/">&times;</a><strong>Maaf, Movie yang dicari tidak ada</strong>')}},error:function(){b.html('<a class="close" href="/">&times;</a><strong>Error! Coba Lagi...</strong>')}});return!1});a.on("click",".close",function(){b.slideUp(500,"easeInExpo");return!1})})(jQuery)
